# olab_camera Developer Guide

**Version:** 1.0
**Last Updated:** 2026-01-29

This guide explains how to extend the `olab_camera` module by adding new camera classes or new feature classes. It can be used as reference documentation for human developers or as a prompt for Claude Code.
...
The `olab_camera` module uses an **object-oriented architecture** with:
...
class CameraYourName(Camera):
    ...
    Args:
        ...
        logger: Optional olab_utils.Logger instance
...
```python
    def _thread_capture(self):
        ...
        try:
            ...
            self.logger.log(f'Starting capture thread', severity=olab_utils.SEVERITY_INFO)
            ...
            while self.camOn:
                ...
                if frame is not None:
                    ...
                    self.logger.log('Failed to capture frame', severity=olab_utils.SEVERITY_WARNING)
            ...
            self.logger.log(f'Error in capture thread: {e}', severity=olab_utils.SEVERITY_ERROR)
        finally:
...
```python
    def start(self, doStream=False, port=8000, doROSpub=False):
        ...
        try:
            ...
            self.logger.log('Camera started', severity=olab_utils.SEVERITY_INFO)
            ...
            self.logger.log(f'Error starting camera: {e}', severity=olab_utils.SEVERITY_ERROR)
```
...
```python
    def shutdown(self):
        ...
        """
        try:
            self.logger.log('Shutting down camera', severity=olab_utils.SEVERITY_INFO)
            ...
            self.logger.log('Camera shutdown complete', severity=olab_utils.SEVERITY_INFO)

        except Exception as e:
            self.logger.log(f'Error during shutdown: {e}', severity=olab_utils.SEVERITY_ERROR)
```
...
See `CameraUSB` in `olab_camera.py` for a complete, working example that supports:
- USB cameras
- RTSP/HTTP streams
- Video files
...
def addYourFeature(self, idName='default', res_rows=None, res_cols=None,
    ...
    try:
        ...
        self.logger.log(f'Error in addYourFeature: {e}', severity=olab_utils.SEVERITY_ERROR)
        return None
```

### Complete Feature Class Example

See `_Aruco` or `_Ultralytics` in `olab_camera.py` for complete, working examples.
...
### Current State
The current `olab_camera.py` is ~3600 lines in a single file. This works but can be difficult to maintain.
...
4. **Export public API** in `__init__.py`:
   ...
   ```
5. **Update client code** to use: `from olab_camera import CameraUSB`
...
#### Log a message:
```python
self.camObject.logger.log('Message here', severity=olab_utils.SEVERITY_INFO)
```
...
- **Existing Examples**: See `CameraUSB`, `_Aruco`, `_Ultralytics` in `olab_camera.py`
...
2. Review this guide
3. Check the module docstrings in `olab_camera.py`
