#!/usr/bin/env python3
"""Run local RF-DETR object detection on a USB camera.

Example:
    python examples/rfdetr_local_detect.py --weights /opt/models/rf-detr-small.pth
"""
...
def main():
    ...
    camera.addRFDETR('detect', task='detect', model_variant=args.variant,
                     weights_path=weights_path, fps_target=args.fps, device=args.device)
    ...
    try:
        ...
        while time.monotonic() < deadline:
            ...
            if feature.deque:
                ...
                for label, confidence, box in zip(latest['class'], latest['class_conf'], latest['xyxy']):
                    print(f'{label:16} {confidence:.2f} {box}')
            time.sleep(1)
